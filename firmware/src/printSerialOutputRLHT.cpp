...
#define SLICE_PRINT(...) Serial.print(__VA_ARGS__)
#define SLICE_PRINTLN(...) Serial.println(__VA_ARGS__)
#else
#define SLICE_PRINT(...)
...
void printSerialOutput()
{
    if (millis() - timing.lastSerialPrint >= SERIAL_UPDATE_TIME_MS)
    {
        ...
        SLICE_PRINT(slice.mode == CLOSED_LOOP ? F("CLOSED_LOOP") : F("OPEN_LOOP"));
        SLICE_PRINT(F(", T1:"));
